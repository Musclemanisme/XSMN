...
class Light_XSMB(nn.Module):
    def __init__(self, in_channels=48, distillation_rate= 1 / 3):
        super(Light_XSMB, self).__init__()
        self.distilled_channels = int(in_channels * distillation_rate)
        self.head = conv_layer(in_channels, in_channels, 3, dilation=1,groups=48)
        self.inter = conv_layer(in_channels, in_channels,1)

        self.c1 = conv_layer(in_channels, self.distilled_channels, 3, dilation=6,)
        self.d1 = conv_layer(self.distilled_channels, self.distilled_channels, 1)

        self.c2 = conv_layer(self.distilled_channels, self.distilled_channels, 3, dilation=2, groups=self.distilled_channels)
        self.d2 = conv_layer(self.distilled_channels, self.distilled_channels, 1)

        self.c3 = conv_layer(self.distilled_channels, self.distilled_channels, 3, dilation=3, groups=self.distilled_channels)
        self.d3 = conv_layer(self.distilled_channels, self.distilled_channels, 1)

        self.att1 = CrossAttention()
        self.att2 = CrossAttention()
        self.att3 = CrossAttention()
    ...
    def forward(self, input):
        in_ = self.inter(self.head(input))
        in_ = self.act(in_)

        c1_out = self.act(self.c1(in_))
        c1_out = self.d1(c1_out)
        ####此处为Ghost操作，对空洞率为6的卷积层结果feature map进行其他的空洞卷积
        c2_out = self.act(self.c2(c1_out))
        c2_out = self.d2(c2_out)
        c3_out = self.act(self.c3(c1_out))
...
class XSMN_Lite(nn.Module):
    def __init__(self, in_channels=12, out_channels=12):
        ...
        self.net1 = nn.Sequential(*[Light_XSMB() for _ in range(1)])
        self.net2 = nn.Sequential(*[Light_XSMB() for _ in range(5)])
