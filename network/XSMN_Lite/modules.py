...
class TransWhiten(nn.Module):
    def __init__(self):
        super(TransWhiten, self).__init__()
        self.norm = nn.LayerNorm([9,512,1024],elementwise_affine=False)

    def forward(self,x):
        # x.shape = B C H W, H*W = N
        B, C, H, W = x.shape
        x_trans = x.reshape(B, C, -1)
        x_mT = x_trans.mT  # B, N, C
        x_weight = torch.bmm(x_trans, x_mT) # B, C, C
        x_weight = torch.softmax(x_weight, dim=2)
        x_modulated = torch.bmm(x_weight, x_trans)
        out = x_modulated.reshape(B, C, H, W)
